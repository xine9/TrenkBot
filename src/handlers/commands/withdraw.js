...
//      Number.isInteger(amount) &&
      ...
      if (amount < withdrawLimit) {
        ...
        //Not enough
        msg += `Wrong amount ${args[0]}, you don't have enough 🔸*TRENK*🔸:${
          wallet.honkPoints
        }`;
...
const withdrawValidation = async (ctx, session, amount, destSLPaddr) => {
  ...
  if (isSLPAddr) {
    ...
    if (tokenBalance < amount) {
      ...
      let warnMsg =
        "ALERT! Escrow wallet doesn't have enough tokens to make transaction";
      warnMsg += `\nEscrow balance: ${tokenBalance} Trenk; Amount: ${amount}`;
    ...
    if (withdrawResult) {
      return `Congratulations! Successfuly withdraw  *${amount}* to SLPaddress:\n*${destSLPaddr}*`;
    } else {
